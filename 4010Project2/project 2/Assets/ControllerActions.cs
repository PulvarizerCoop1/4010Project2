...
public class ControllerActions : MonoBehaviour
{
	...
	void Bird(){
		// each iteration of a loop move and flap sound

		// max height 8 maybe
		// use Thread.Sleep(milliseconds) to pause
		// for loop fly up

		// for loop move to new loc
		// for looc down
	}
    ...
    void Update()
	{
		...
		if (Controller.GetHairTriggerDown () ) {	//&& canTP
			...
			// calculate player position on the tile and the tiles positions to the parent origin

			// ray cast to get the new tile
